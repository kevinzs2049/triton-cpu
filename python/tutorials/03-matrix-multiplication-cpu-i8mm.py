...
DTYPE = getattr(torch, (os.getenv("DTYPE", "int8")))
IS_INT8 = DTYPE == torch.int8
# Choose block size depending on dtype.
BLOCK_SIZE_M = 32 if IS_INT8 else (8 if DTYPE == torch.float32 else 32)
BLOCK_SIZE_N = 32
BLOCK_SIZE_K = 16 if IS_INT8 else (8 if DTYPE == torch.float32 else 32)
...
def matmul_kernel(
    ...
    accumulator = tl.zeros((BLOCK_SIZE_M, BLOCK_SIZE_N), dtype=tl.int32 if IS_INT8 else tl.float32)
...
def matmul_preprocess_input(a: torch.Tensor, b: torch.Tensor, c: torch.Tensor, num_threads=0):
    ...
    if c is None:
        # Allocates output.
        c = torch.empty((M, N), device=a.device, dtype=torch.int32 if IS_INT8 else torch.float32)
    else:
...
a = torch.randint(-128, 127, (512, 512), device='cpu', dtype=DTYPE) if IS_INT8 else torch.randn(
    (512, 512), device='cpu', dtype=DTYPE)
b = torch.randint(-128, 127, (512, 512), device='cpu', dtype=DTYPE) if IS_INT8 else torch.randn(
    (512, 512), device='cpu', dtype=DTYPE)
c = None
torch_output = torch.matmul(a.int(), b.int()) if IS_INT8 else torch.matmul(a.to(torch.float32), b.to(torch.float32))
if PREPACKED:
...
LINE_VALS = ['triton-cpu-single', 'triton-cpu'] if IS_INT8 else ['triton-cpu-single', 'triton-cpu', 'torch-cpu-native']
LINE_NAMES = ['TritonCPU 1', 'TritonCPU'] if IS_INT8 else ['TritonCPU 1', 'TritonCPU', 'TorchCPU (native)']
LINE_STYLES = [('blue', '--'), ('blue', '-')] if IS_INT8 else [('blue', '--'), ('blue', '-'), ('green', '--')]
...
def benchmark(M, N, K, provider):
    ...
    if provider == 'torch-gpu':
        ...
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: torch.matmul(a.int(), b.int(), out=c) if IS_INT8 else torch.matmul(a, b, out=c),
            quantiles=quantiles)
